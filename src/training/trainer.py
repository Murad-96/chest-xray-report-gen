...
class Trainer:
    ...
    def _validate(self, epoch: int) -> tuple[dict, list]:
        ...
        if hypotheses and self.compute_linguistic_metrics is not None:
            try:
                ...
                # Prefix with val_ for clean metric naming
    ...
    def _log_epoch(
        ...
        # Log a few sample generations to the console for qualitative inspection.
        # (Per-epoch sample inspection was the key debugging signal in the paper,
        # far more informative than the loss curve alone for this task.)
        for img_id, ref, hyp in sample_rows[:3]:
            logger.info(f"  sample [{img_id}]")
            logger.info(f"    ref: {ref}")
            logger.info(f"    gen: {hyp}")
